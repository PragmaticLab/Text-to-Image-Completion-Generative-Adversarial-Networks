...
def build_lm(text_inputs, num_symbols, cond, seq_len=None):
  if seq_len is None:
    # print(_seq_len(text_inputs)[0] == 1)
  ...
  with tf.variable_scope("decoder"):
    ...
    # else:
      output, state = tf.nn.dynamic_rnn(cell, rnn_inputs,
          initial_state = cond,
          dtype=tf.float32,
          sequence_length=tl.layers.retrieve_seq_length_op2(text_inputs))
...
def lm_loss(text, num_symbols, cond):
  ...
  xent = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits, labels=text_targets)
